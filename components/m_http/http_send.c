...
esp_err_t http_send(http_task_struct* m_task_struct)
{
    ...
    { 
        ...
        snprintf(body, sizeof(body),
            ...
            "2\r\n"
            "--%s--\r\n",
            m_boundary, m_boundary, m_boundary, m_boundary);
    ...
    else if(m_task_struct->task==BINDDEVICE)
    {
        ...
        snprintf(short_mac, sizeof(short_mac), "%c%c%c%c%c%c",
                ...
                m_task_struct->parament[0][12], m_task_struct->parament[0][13], // "78"
                m_task_struct->parament[0][15], m_task_struct->parament[0][16]  // "56"
        );
        char name[25];
        sprintf(name, "Reminder-%s", short_mac);
        // 设置 Content-Type
    ...
    if (err != ESP_OK) 
    {
        ESP_LOGE(HTTP_TAG, "HTTP POST request failed");
    }
